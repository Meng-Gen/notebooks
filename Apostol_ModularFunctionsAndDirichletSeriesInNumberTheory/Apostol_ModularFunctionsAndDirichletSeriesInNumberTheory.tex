...
\emph{Find all elements $A$ of $\Gamma$ which}
\begin{enumerate}
\item[(a)]
  \emph{commute with $S$;}

\item[(b)]
  \emph{commute with $ST$.} \\
\end{enumerate}
...
\emph{Proof of (a).}
  \begin{align*}
    ...
    \begin{pmatrix}
      a & b \\
      c & d
    \end{pmatrix} \in \Gamma$ satisfying $AS = SA$} \\
    \Longrightarrow & \:
    \begin{pmatrix}
      a & b \\
      c & d
...
\emph{Proof of (b).}
  \begin{align*}
    ...
    \begin{pmatrix}
      a & b \\
      c & d
    \end{pmatrix} \in \Gamma$ satisfying $A(ST) = (ST)A$} \\
    \Longrightarrow & \:
    \begin{pmatrix}
      a & b \\
      c & d
    ...
    a = -c+d, b = -c \\
    \Longrightarrow & \:
    1 = \det(A) = ad - bc = (-c+d)d + c^2 = c^2 - cd + d^2 \\
    \Longrightarrow & \:
    4 = (2 c - d)^2 + 3d^2 \\
    \Longrightarrow & \:
    (c, d) = (\pm 1, 0), (0, \pm 1), \pm (1 , 1) \\
    \Longrightarrow & \:
    A = I, ST, (ST)^2.
  \end{align*}
  (Here we identify each matrix with its negative.)
...
\emph{Proof.}
  Same as Exercise 2.1 (b).
  Note that $1 + \rho + \rho^2 = 0$.
  Hence
  \begin{align*}
    & \:
    \text{$A =
    ...
    \Longrightarrow & \:
    A\rho = \frac{a\rho + b}{c\rho + d} = i \\
    \Longrightarrow & \:
    a\rho + b = c\rho^2 + d\rho = c(-\rho-1) + d\rho = (-c+d)\rho - c \\
    \Longrightarrow & \:
    a = -c+d, b = -c. \\
    \Longrightarrow & \:
    1 = \det(A) = ad - bc = (-c+d)d + c^2 = c^2 - cd + d^2 \\
