...
\emph{Determine all elements $A$ of $\Gamma$ which leave $i$ fixed.} \\
...
\begin{enumerate}
\item[(1)]
  \emph{Show that $A = I, S \in \Gamma$ fixes $i$.}
  Say
  $A =
  \begin{pmatrix}
    a & b \\
    c & d
  \end{pmatrix} \in \Gamma$ satisfying $Ai = i$.
  Then $Ai = \frac{ai + b}{ci + d} = i$ implies that
  $ai + b = ci^2 + di = di - c$ or $b = -c$ and $a = d$.
  Hence $1 = \det(A) = ad - bc = a^2 + b^2$.
  Therefore $(a, b) = (\pm 1, 0)$ or $(0, \pm 1)$.

\item[(2)]
  The case $a = \pm 1$ and $b = 0$.
  $A = I = \begin{pmatrix}
    1 & 0 \\
    0 & 1
  \end{pmatrix}$
  (by identifying each matrix with its negative).

\item[(3)]
  The case $b = \pm 1$ and $a = 0$.
  $A = S = \begin{pmatrix}
    0 & -1 \\
    1 & 0
  \end{pmatrix}$
...
\emph{Determine all elements $A$ of $\Gamma$ which leave $\rho = e^{\frac{2\pi i}{3}}$ fixed.} \\
...
\begin{enumerate}
\item[(1)]
  \emph{Show that $A = I, S \in \Gamma$ fixes $\rho$.}
  Say
  $A =
  \begin{pmatrix}
    a & b \\
    c & d
  \end{pmatrix} \in \Gamma$ satisfying $A\rho = \rho$.
  Then $A\rho = \frac{a\rho + b}{c\rho + d} = \rho$ implies that
  $a\rho + b
    = c\rho^2 + d\rho
    = c(-\rho-1) + d\rho
    = (-c+d)\rho - c$
...
\item[(2)]
  So $1 = \det(A) = ad - bc = (-c+d)d + c^2 = c^2 - cd + d^2$,
  or $1 = \left( c - \frac{d}{2} \right)^2 + \frac{3}{4}d^2$,
  or $4 = (2 c - d)^2 + 3d^2$.
  Hence $(c, d) = (\pm 1, 0)$, $(0, \pm 1)$.

\item[(3)]
  The case $c = \pm 1$ and $d = 0$.
  $A = S = \begin{pmatrix}
    0 & -1 \\
    1 & 0
  \end{pmatrix}$
...
\addcontentsline{toc}{subsection}{Quadratic forms and the modular group}
...
\addcontentsline{toc}{subsubsection}{Supplement 2.15.1. (Chinese remainder theorem)}
