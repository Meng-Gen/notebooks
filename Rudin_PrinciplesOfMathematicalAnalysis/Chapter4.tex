...
Since $y \in f(X)-f(E)$, there exist a point $x \in X-E$ such that $y = f(x)$.
Since $E$ is dense in $X$, there is a sequence $\{x_n\}$ in $E$ such that
...
Given any $p \in X-E$, there is a sequence $\{p_n\}$ in $E$ such that
...
\begin{enumerate}
  ...
  (The case $\lim_{x \to p^{-}} g(x)$ is similar.)
    \begin{enumerate}
    \item[(i)]
    For such $\epsilon > 0$, there is a $\delta' > 0$ such that
    ...
    By (i),
    \begin{align*}
      f(p) - \epsilon &< f(a_i) < f(p) + \epsilon \text{ and } \\
      f(p) - \epsilon &< f(b_i) < f(p) + \epsilon, \\
      f(p) - \epsilon < f(a_i) &\leq g(x) \leq f(b_i) < f(p) + \epsilon \text{ or } \\
      f(p) - \epsilon < f(b_i) &\leq g(x) \leq f(a_i) < f(p) + \epsilon.
    \end{align*}
    \end{enumerate}
  Hence, given $\epsilon > 0$
  there is a $\delta > 0$ such that
  $|g(x) - g(p)| < \epsilon$ whenever $p < x < \delta$ (and $x \in \mathbb{R}^1$),
...
\end{enumerate}
In any cases, the conclusion holds.
$\Box$ \\\\
...
\emph{The function $[x]$ only has discontinuities at $x \in \mathbb{Z}$.}
  ...
  Given any $\epsilon > 0$, there is a $\delta = \min\{p-n, (n+1)-p\} > 0$
  such that $\abs{[x] - [p]} < \epsilon$
  whenever $\abs{x - p} < \delta$.
  In fact, $\abs{x - p} < \delta$ is equivalent to $n < x < n+1$
  and therefore $\abs{[x] - [p]} = \abs{n - n} = 0 < \epsilon$.
  \item[(b)]
