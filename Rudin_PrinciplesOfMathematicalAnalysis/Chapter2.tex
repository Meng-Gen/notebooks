...
\emph{Show that every second-countable metric space $X$ is separable.}
  \begin{enumerate}
  \item[(a)]
  Let $\mathscr{B} = \{ B_i : i \in \mathbb{Z}^+ \}$ be a countable base of $X$.
  \item[(b)]
  For every $B_i \in \mathscr{B}$, pick any point $p_i$ of $B_i$
  and collect them as $$E = \{ p_i : p_i \in B_i \text{ for } i \in \mathbb{Z}^+ \}.$$
  \item[(c)]
  $E$ is countable.
  \item[(d)]
  \emph{Show that $E$ is dense.}
