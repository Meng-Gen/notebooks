...
(Note that $\alpha$ is not necessary increasing strictly,
the equality does not hold only if $f^p = g^q$ as $\alpha$ is constant on some subinterval
of $[a,b]$.)
$\Box$ \\
...
\item[(3)]
  ...
  \]
  (Note that $\alpha$ is not necessary increasing strictly,
  the equality does not hold only if
  $\frac{|f|^p}{\int_{a}^{b} |f|^p d\alpha} = \frac{|g|^q}{\int_{a}^{b} |g|^q d\alpha}$
  as $\alpha$ is constant on some subinterval of $[a,b]$.
  It implies that if $\alpha$ is increasing strictly, $\alpha(x) = x$ for example,
