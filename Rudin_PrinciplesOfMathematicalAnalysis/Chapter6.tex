...
\begin{enumerate}
  ...
  \]
  Since $\cos u$ has two absolute minimums or maximums at two different points
  $u = e^x$ and $u = e^{x+1}$, by the property of $\cos(u)$ there is some
  $u_0 \in [e^x,e^{x+1}]$ such that $\cos(u_0) = 0$.
  Hence given $\varepsilon = \frac{1}{2} > 0$ there exists $\delta > 0$
  such that
  \[
    |\cos(u)| \leq \frac{1}{2}
  ...
  whenever
  \[
    u \in E = [\max\{u_0-\delta,e^x\},\min\{u_0+\delta,e^{x+1}\}]
    \subseteq [e^x,e^{x+1}].
  \]
  Here $|E| > 0$.
  So that
  \[
    \int_{\exp(x)}^{\exp(x+1)} \frac{|\cos u|}{u^2} du
    \int_{\exp(x)}^{\exp(x+1)} \frac{1}{u^2} du
      - \frac{1}{2} \int_{E} \frac{1}{u^2} du
    < \int_{\exp(x)}^{\exp(x+1)} \frac{1}{u^2} du,
  \]
  which is absurd.
  Hence the equality in (2) does not hold, or $e^x|f(x)| < 2$.
