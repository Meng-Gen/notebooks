...
given any $\epsilon > 0$, there is $N$ such that
$\abs{s_n - s} < \epsilon$ whenever $n \geq N$.
So
$$\abs{\abs{s_n}-\abs{s}} \leq \abs{s_n - s}  < \epsilon$$
...
\emph{Proof ($\epsilon-N$ argument).}
Let $s_n = \sqrt{n^2 + n} - n$.
\emph{Show that the sequence $\{s_n\}$ converges to $s = \frac{1}{2}$.}
Given any $\epsilon > 0$, there is $N > \frac{1}{\epsilon}$ such that
\begin{align*}
\abs{s_n - s}
